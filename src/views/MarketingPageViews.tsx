    ...
    <MarketingPageLayout title="ปรึกษา Baawork" subtitle="ส่งโจทย์ ระบบที่อยากทำ หรือปัญหาที่อยากแก้มาให้เราเริ่มดูภาพรวมร่วมกัน">
      <MarketingContentSection backgroundColor="#FFFFFF" variant="slide-right">
        ...
        <Box component="form" onSubmit={handleSubmit} sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, minmax(0, 1fr))' }, gap: { xs: 2, md: 2.5 }, ...cardSx }}>
          {[['ชื่อของคุณ', 'name', 'text', 'name'], ['อีเมลสำหรับติดต่อกลับ', 'email', 'email', 'email'], ['เบอร์โทรศัพท์ (ถ้ามี)', 'phone', 'tel', 'tel'], ['บริษัทหรือองค์กร (ถ้ามี)', 'company', 'text', 'organization']].map(([label, name, type, autoComplete]) => <Box key={name} component="label" sx={{ display: 'grid', gap: 0.8, fontSize: 14, fontWeight: 700 }}>{label}<Box component="input" name={name} type={type} autoComplete={autoComplete} required={name === 'name' || name === 'email'} sx={inputSx} /></Box>)}
          <Box component="label" sx={{ gridColumn: '1 / -1', display: 'grid', gap: 0.8, fontSize: 14, fontWeight: 700 }}>รายละเอียดที่อยากปรึกษา<Box component="textarea" name="message" autoComplete="off" required rows={5} sx={{ ...inputSx, p: 1.5, resize: 'vertical' }} /></Box>
