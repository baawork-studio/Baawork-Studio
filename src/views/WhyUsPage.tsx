...
function SectionHeading({ title, description }: { title: string; description: string }) {
  return <Stack spacing={{ xs: 1.5, md: 2 }} sx={{ mb: { xs: 4, md: 5 } }}><Typography component="h2" sx={{ ...typeScale.sectionTitle, color: palette.text }}>{title}</Typography><Typography sx={{ ...typeScale.bodyLarge, color: '#4B5563', maxWidth: 760 }}>{description}</Typography></Stack>;
}

export function WhyUsPage() {
  return (
    <DetailStylePage title="ทำไมต้องเรา" subtitle="เพราะระบบที่ดีต้องเชื่อมงานออกแบบ ประสบการณ์ใช้งาน และเทคโนโลยีเข้ากับธุรกิจจริง">
      <DetailStyleSection backgroundColor="#FFFFFF" variant="slide-right">
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3, minmax(0, 1fr))' }, gap: { xs: 2, md: 2.5 } }}>
          {principles.map(([title, description]) => <Stack key={title} spacing={2} sx={{ ...cardSx, minHeight: { xs: 220, md: 280 } }}><Typography sx={{ ...typeScale.cardTitle, color: palette.primaryPink }}>{title}</Typography><Typography sx={{ ...typeScale.body, color: '#4B5563' }}>{description}</Typography></Stack>)}
        </Box>
      </DetailStyleSection>
      ...
      <DetailStyleSection backgroundColor="#FFFFFF" variant="scale">
        ...
        <Stack spacing={1.5}>{outcomes.map((item) => <Box key={item} sx={cardSx}><Typography sx={{ ...typeScale.bodyLarge, color: palette.text, fontWeight: 700 }}>{item}</Typography></Box>)}</Stack>
      </DetailStyleSection>
    </DetailStylePage>
  );
}
